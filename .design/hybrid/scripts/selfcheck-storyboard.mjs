...
await section('3. алгоритм по ветвям дерева', async () => {
  ...
  note(
    RED ? studioCount < figs.length : studioCount === figs.length,
    `бонус места «в студии» держит всю выдачу: студийных ${studioCount} из ${figs.length} (пул 17 кадров в 13 сериях): ${figs.map((f) => f.place).join(',')}`,
  );
