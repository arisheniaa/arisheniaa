...
await section('3. алгоритм по ветвям дерева', async () => {
  ...
  // 3.2 — материал БОЛЬШЕ НЕ ОСЛАБЛЯЕТСЯ (Ф37, отменяет прежнее поведение):
  //       творческая+плёнка = 0 кадров в архиве — раньше это давало «ослабленный»
  //       показ цифровых кадров с оговоркой, теперь честно 0 результатов, без
  //       подмены материала и без сообщения «набралось мало».
  await page.goto(BASE, { waitUntil: 'networkidle' });
  ...
  note(
    RED ? figs.length !== 0 : figs.length === 0,
    `результат честно пуст, материал не подменён цифрой (в архиве творческая+плёнка нет вовсе): ${figs.length} кадров`,
  );
  note(relaxNote.includes('не нашлось подходящих кадров'), 'страница показывает честное состояние «нет результатов», не тихую подмену материала');
...
await section('4. Pinterest по ветвям', async () => {
  ...
  // «вдвоём» — правка Ф38: теперь ТРИ доски, «запечатлеваем-любовь» первая и
  // активная по умолчанию (новая привязка, не было в Ф34); макияж/луки по-
  // прежнему НЕ подключаются к «вдвоём» (Ф34 дословно: «в парную не стоит
  // это добавлять» — эта часть не отменена, касалась только двух других досок)
  await page.goto(BASE, { waitUntil: 'networkidle' });
  await walkPath(page, ['Вайб имеется', 'Вдвоём', 'Цифра', 'В студии', 'Да, уже есть']);
  tabs = await tabsOf();
  const expectDuo = ['Запечатлеваем любовь', 'Inspiration', 'Восхищает'];
  note(
    RED ? tabs.length !== 3 : tabs.length === 3 && tabs.every((t, i) => t === expectDuo[i]),
    `«вдвоём» — 3 доски, «Запечатлеваем любовь» первая (Ф38) (получено: ${tabs.join(', ')})`,
  );
