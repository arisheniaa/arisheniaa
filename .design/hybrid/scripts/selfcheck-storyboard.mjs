...
await section('3. алгоритм по ветвям дерева', async () => {
  ...
  note(
    RED ? figs.length === 2 : figs.length === 6,
    `фильтр «парень» ослаблен на плёнке: без ослабления было бы 2 кадра, получено ${figs.length}`,
  );
  ...
  const maleCount = figs.filter((f) => f.gender === 'парень').length;
  note(
