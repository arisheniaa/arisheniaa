...
/* ─── 2b. Свечение звёзд: один рецепт на все вводы, и оно не мигает ───────
   (Ф55 → перевёрнуто Ф56 → перевёрнуто снова Ф57)

   ЭТУ ПРОВЕРКУ ПЕРЕПИСЫВАЛИ ТРИЖДЫ, И ЭТО САМО ПО СЕБЕ НАХОДКА. Сначала она
   требовала, чтобы свечение на тач-вводе КОЛЕБАЛОСЬ (Ф55 прочитала «не
   горят» как «не хватает движения»). Потом — чтобы оно было РОВНЫМ и
   заметно бо́льшим по радиусу (Ф56, после «ты сделал сверкающие мигающие
   звёздочки»). Оба раза утверждение формулировалось про ЧИСЛО, оба раза
   число росло, и оба раза владелица говорила, что на сайте всё по-прежнему.

   Причина в том, что число было не про то. Радиус ореола `drop-shadow` — не
   мера яркости: чем он больше, тем шире размазан один и тот же свет и тем
   ТУСКЛЕЕ каждый пиксель. Проверка, требовавшая радиус побольше, добросовестно
   зеленела ровно на той правке, которая делала хуже.

   ПОЭТОМУ ТЕПЕРЬ УТВЕРЖДАЕТСЯ НЕ ВЕЛИЧИНА, А ТОЖДЕСТВО. Владелица назвала
   правильным то, что видит на ноутбуке; значит проверяемое свойство — что
   телефон получает ТОТ ЖЕ рецепт, а не свой собственный, каким бы
   «улучшенным» он ни казался. Это утверждение невозможно удовлетворить,
   двигая числа в неверную сторону: любое расхождение рецептов красит его.

   Второе утверждение — что свечение не мигает — остаётся от Ф56: оно всё
   ещё сторожит возврат пульсации.

   Обе ветки снимаются на ОДНОЙ ширине 390 px с подменённым `matchMedia`:
   одна и та же россыпь (шум детерминированный), отличается только ответ
   про наличие курсора. Сравнивать телефон с настоящим десктопом было бы
   нечестно — там другая ширина, другое число звёзд и другие размеры. */
{
  const снять = async (hover) => {
    ...
    const фильтр = () =>
      page.evaluate(() => {
        ...
        return g ? { filter: g.style.filter, opacity: g.style.opacity } : null;
      });

    const ряд = [];
    for (let i = 0; i < 6; i++) {
      const f = await фильтр();
      ряд.push(f ? Number(/drop-shadow\(0 0 ([\d.]+)px/.exec(f.filter)?.[1] ?? 0) : null);
      if (i < 5) await page.waitForTimeout(400);
    }
    const итог = await фильтр();
    await ctx.close();
    return { ...итог, ряд: ряд.filter((v) => v !== null) };
  };
  ...
  note(
    RED ? тач.filter !== курсор.filter : тач.filter === курсор.filter,
    `рецепт свечения на тач-вводе тот же, что под курсором (тач: ${(тач.filter || '—').slice(0, 46)}…)`,
  );
  note(
    RED ? тач.opacity !== курсор.opacity : тач.opacity === курсор.opacity,
    `плотность звезды на тач-вводе та же, что под курсором (${тач.opacity} против ${курсор.opacity})`,
  );

  const размах = тач.ряд.length ? Math.max(...тач.ряд) - Math.min(...тач.ряд) : 0;
  note(
    RED ? размах > 0.5 : размах <= 0.5,
    `свечение РОВНОЕ, не мигает: за 2 с радиус изменился на ${размах.toFixed(1)}px (${тач.ряд.join('/')})`,
  );
}
