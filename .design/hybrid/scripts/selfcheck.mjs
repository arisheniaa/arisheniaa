...
/* ─── 2b. На тач-вводе звёзды горят РОВНО и заметно (Ф55, перевёрнуто Ф56) ─
   Проверка «есть drop-shadow» выше была правдой и на телефоне — и всё равно
   владелица видела «не горят». Значит одного свойства в CSSOM тут мало.

   ПРОВЕРКА ПЕРЕВЁРНУТА, А НЕ ПОДПРАВЛЕНА, И ВОТ ПОЧЕМУ. Ф55 читала жалобу
   как «не хватает движения» и заводила каждой звезде медленный вдох-выдох;
   эта проверка тогда требовала, чтобы радиус ореола КОЛЕБАЛСЯ. Владелица
   посмотрела и сказала обратное: «ты сделал сверкающие мигающие звёздочки,
   но я хотела чтобы ты сделал их горящими как на ноутбуке… сами по себе
   тоже горящие». Требование сменилось на противоположное, поэтому и
   утверждение стало противоположным — но не слабее: теперь их два сразу,
   и оба обязаны выполниться.

    1. ГОРЯТ ЗАМЕТНО. Радиус ореола на тач-вводе выше 12 px. Порог не с
       потолка: у средней звезды (размер ~21 px) базовый ореол без добавки —
       9,7 px, с добавкой — около 16. Двенадцать лежит между ними, то есть
       проверка отличает «добавка есть» от «добавки нет», а не просто
       «фильтр непустой».
    2. ГОРЯТ РОВНО. Радиус НЕ меняется за три секунды наблюдения. Допуск
       0,5 px — ниже кванта записи фильтра (0,08 свечения ≈ 1,8 px), то есть
       любое настоящее мерцание проверка поймает, а шума округления здесь
       быть не может: значение пишется один раз и больше не трогается.

   Меряем одну звезду, а не среднее по полю: среднее скрыло бы и то, и
   другое — при разных фазах его размах был 0,23 px против 5,3 у отдельной
   звезды (замерено на прежней, мигающей редакции). */
{
  ...
  const радиусМин = чистый.length ? Math.min(...чистый) : 0;
  note(
    RED ? радиусМин <= 12 : радиусМин > 12,
    `на тач-вводе звезда горит заметно: радиус ореола ${радиусМин.toFixed(1)}px при пороге 12`,
  );
  note(
    RED ? размах > 0.5 : размах <= 0.5,
    `на тач-вводе свечение РОВНОЕ, не мигает: за 3 с радиус изменился на ${размах.toFixed(1)}px (${чистый.join('/')})`,
  );
  await ctx.close();
}
