...
export function useRack(count = 2): RackApi {
  ...
  useEffect(() => {
    ...
    const onUp = (e: PointerEvent) => {
      ...
      if (projected > W * THRESH) {
        /* ВПРАВО — ВПЕРЁД (Ф62, «при пролистывании вправо они листаются
           вперёд как и было задумано изначально»). Кадр уходит туда, куда
           шёл палец, и следующий становится верхним — как и раньше, только
           направление теперь закреплено за смыслом. */
        l.tx = W * 1.5;
        l.damp = 1; // Ф61: было 0.8 — овершут в конце читался щелчком
        l.resp = 0.52; // Ф61: было 0.38
        flying.current = t;
      } else if (projected < -W * THRESH) {
        /* ВЛЕВО — НАЗАД (Ф62, «при пролистывании влево возвращать предыдущую
           фотографию»). Здесь ничего не улетает: возвращается тот кадр,
           который ушёл в прошлый раз.

           ОТКУДА ОН ПРИХОДИТ. Вперёд кадры уходят ВПРАВО, значит вернуться
           предыдущий обязан оттуда же — иначе он «телепортируется» с другой
           стороны. Заходя справа, он движется влево, то есть туда же, куда
           идёт палец: жест и картинка совпадают в обоих направлениях.

           СМЕНА ВЕРХНЕГО ПРОИСХОДИТ СРАЗУ, а не по достижении порога, как у
           броска вперёд. У броска смена отложена: пока улетающий кадр не
           ушёл за 62 % ширины, он ещё виден и обязан оставаться верхним.
           Здесь наоборот — возвращающийся кадр обязан быть верхним с первого
           же пикселя, иначе он въезжал бы ПОД стопкой.

           Текущий верхний при этом не выбрасывается: ему просто ставится
           цель «домой», и он уезжает под возвращающийся как обычный слой. */
  ...
  const advance = useCallback(() => {
    ...
    const l = L.current[t];
    /* НАПРАВЛЕНИЕ РАЗВЁРНУТО Ф62: вправо, а не влево. Раньше «вперёд» не было
       привязано к стороне — жест в любую сторону листал вперёд, и кнопка
       бросала кадр влево просто «как листают». Теперь стороны заняты смыслом
       (вправо — вперёд, влево — назад), и кнопка «следующий кадр» обязана
       показывать то же движение, что жест вперёд. Иначе один и тот же
       переворот выглядел бы зеркально в зависимости от того, чем вызван. */
    l.tx = W * 1.5;
