...
/** Доля ширины, за которой улетающий вперёд кадр перестаёт быть верхним и
 *  пропадает: дальше он всё равно закрыт, а второй уже целиком сверху.
 *  Ф64 дала этому числу имя, потому что у него появился второй читатель —
 *  возврат «назад» проявляется ровно в этой же точке, и мирроринг должен быть
 *  виден в коде, а не совпадать по случайности двух литералов. */
const FLIP = 0.62;
/** Откуда приезжает возвращаемый кадр, в долях ширины (Ф64). Ровно одна
 *  ширина: кадр стоит вплотную слева от стопки, целиком вне её, — то есть в
 *  тот момент, когда он появляется, он ничего собой не накрывает. */
const BACK_FROM = 1.0;
...
/** Проявление въезжающего кадра (Ф64): 0 на расстоянии `BACK_FROM` ширин от
 *  дома, 1 — на `FLIP`, ровно там, где кадр, улетающий ВПЕРЁД, перестаёт быть
 *  верхним и пропадает. То есть оба направления работают на одной границе:
 *  вперёд кадр в ней исчезает, назад — в ней уже полностью проявился, и
 *  ни один не возникает и не пропадает рывком посреди пустоты. */
const enterFade = (x: number, W: number) => {
  const from = W * BACK_FROM;
  const to = W * FLIP;
  return Math.min(1, Math.max(0, (from - Math.abs(x)) / (from - to)));
};
  ...
  const paint = useCallback(() => {
    ...
    for (let i = 0; i < count; i++) {
      ...
      if (i === t) {
        ...
        // единственный случай, когда верхний кадр не полностью непрозрачен —
        // он въезжает обратно и ещё проявляется (Ф64, `enterFade`)
        node.style.opacity = i === entering.current ? enterFade(l.x, W).toFixed(3) : '1';
