...
export function useRack(count = 2): RackApi {
  ...
  /* ——— «НАЗАД»: один код на жест вправо и на клавиатуру ———
     Ф63 вынесла эту ветвь из обработчика отпускания в отдельную функцию.
     Причина ровно та же, по которой Ф61 свела числа кнопки и жеста: если
     переворот вызывается двумя вводами, он обязан быть ОДНИМ переворотом, а
     не двумя похожими кусками кода, которые разъедутся на следующей правке.
     Прямой сосед `advance` ниже — та же роль для «вперёд».

     Здесь ничего не улетает: возвращается тот кадр, который ушёл в прошлый
     раз.

     ОТКУДА ОН ПРИХОДИТ. Вперёд кадры уходят ВЛЕВО (Ф63), значит вернуться
     предыдущий обязан оттуда же — иначе он «телепортируется» с другой
     стороны. Заходя слева, он движется вправо, то есть туда же, куда идёт
     палец: жест и картинка совпадают в обоих направлениях.

     СМЕНА ВЕРХНЕГО ПРОИСХОДИТ СРАЗУ, а не по достижении порога, как у броска
     вперёд. У броска смена отложена: пока улетающий кадр не ушёл за 62 %
     ширины, он ещё виден и обязан оставаться верхним. Здесь наоборот —
     возвращающийся кадр обязан быть верхним с первого же пикселя, иначе он
     въезжал бы ПОД стопкой.

     Текущий верхний при этом не выбрасывается: ему просто ставится цель
     «домой», и он уезжает под возвращающийся как обычный слой. Скорость ему
     здесь НЕ обнуляется — у жеста она уже влита пальцем строкой выше
     (`l.vx = vel * 0.45`), у клавиатуры её и так нет. */
  ...
  useEffect(() => {
    ...
    const onUp = (e: PointerEvent) => {
      ...
      if (projected < -W * THRESH) {
        ...
        /* ВПРАВО — НАЗАД (Ф63, «листание вправо будет назад»). Механика —
           в `stepBack` выше, общая с клавиатурой. */
        stepBack();
      } else {
        l.tx = 0;
        l.damp = 1;
