...
/* Ф67 — развилка «дизайн / фотография» на входе. Полноэкранная штора поверх
   главной с её текстом дословно (`copy.gate`); «Узнать больше про
   фотографию» роняет штору и открывает ЭТУ страницу, «Узнать больше про
   дизайн» уводит на `/design.html`. Когда показывается, а когда нет —
...
export function App() {
  return (
    <>
      <Gradient />
      {/* Штора развилки (Ф67) — поверх страницы, сама решает, показываться
          ли (адрес с якорем и повторный визит сессии проходят без неё). */}
      <Gate page="home" />
      <Page />
      <Grain />
    </>
