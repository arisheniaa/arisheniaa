...
/* Ф67 — развилка «дизайн / фотография» на входе. Полноэкранная штора поверх
   главной с её текстом дословно (`copy.gate`); «хочу съемку» уводит
   штору вверх и открывает ЭТУ страницу, «хочу сайт» ведёт на
   `/design.html`. Когда показывается, а когда нет —
   расписано в шапке `Gate.tsx`. */
import { Gate } from './Gate';
/* Ф72 — вторая ветка переехала на эту же страницу: `/design.html` больше
   нет, её содержимое вставляется ниже как обычные секции. Разбор — в шапке
   `design/DesignApp.tsx`. */
import { DesignHeader, DesignSections } from './design/DesignApp';
import { dcopy } from './design/copy';
...
/** Ветка сайта: что показано под развилкой. */
type Branch = 'photo' | 'design';

/** Ветка выводится ИЗ АДРЕСА, а не хранится отдельно: один источник правды,
 *  и он же переживает обновление страницы и кнопку «назад». */
const branchOf = (hash: string): Branch => (hash === '#design' ? 'design' : 'photo');

export function App() {
  const [branch, setBranch] = useState<Branch>(() => branchOf(window.location.hash));
  /* Развилка показывается только на ГОЛОМ адресе. Любой якорь — внутренняя
     ссылка, метка ветки или перенаправление со старого `/design.html` — её
     пропускает: человек пришёл в место, а не за вопросом. */
  const [gate, setGate] = useState(() => !window.location.hash);

  /* Кнопки «назад» и «вперёд» в браузере. Выбор ветки — отдельный шаг
     истории (`pushState` ниже), поэтому «назад» с ветки возвращает к
     развилке, а не уводит с сайта. */
  useEffect(() => {
    const onPop = () => {
      const h = window.location.hash;
      setBranch(branchOf(h));
      setGate(!h);
    };
  ...
  /* Заголовок вкладки и описание идут за веткой: на одной странице живут две
     профессии, и вкладка обязана называть ту, которую сейчас смотрят. */
  useEffect(() => {
    const m = branch === 'design' ? dcopy.meta : copy.meta;
  ...
  return (
    <>
      {/* Полотно светлеет к середине только на ветке дизайна — её приём,
          см. `.mesh-veil` в styles.css. */}
      <Gradient veil={branch === 'design'} />
      {gate ? (
        <Gate
          onChoose={(b) => {
            /* Адрес получает метку ветки ОТДЕЛЬНЫМ шагом истории — ради
               кнопки «назад». Ветка меняется сразу, пока штора ещё
               закрывает экран: подмена происходит за ней. */
            history.pushState(null, '', b === 'design' ? '#design' : '#photo');
            setBranch(b);
          }}
          onGone={() => setGate(false)}
        />
      ) : null}
      {/* `key` — намеренно: смена ветки ПЕРЕСОБИРАЕТ страницу, и вместе с
          ней заново запускаются наблюдатели появления секций. Без этого
          секции второй ветки остались бы невидимыми: их проявление
          считается один раз при монтировании. */}
      <Page key={branch} branch={branch} />
      <Grain />
    </>
  );
}

/** Страница целиком: шапка выбранной ветки, общий `<main>` с обеими ветками
 *  и общий подвал. */
function Page({ branch }: { branch: Branch }) {
  useReveal();
  useFocusScrub();
  ...
  return (
    <>
      {/* ШАПКА У КАЖДОЙ ВЕТКИ СВОЯ (решение владелицы при разборе Ф72).
          Пункты фотографической навигации ведут к разделам съёмок — на
          ветке про сайты они вели бы в пустоту, и наоборот. */}
      {branch === 'design' ? <DesignHeader /> : <Header />}
      {/* Боковой индикатор разделов (Ф29 п.4) — fixed, читает `main > section`
          ...
          Только на фотоветке: он считает секции главной, а у ветки дизайна
          их три и лестница из трёх звёзд не сказала бы ничего. */}
      {branch === 'photo' ? <SectionStars /> : null}
      ...
      {/* ОДИН `<main>` НА ОБЕ ВЕТКИ. Второй лендмарк сказал бы скринридеру,
          что на странице два основных содержания, — а оно одно, просто
          зависит от выбора.

          ОБЕ ВЕТКИ ЛЕЖАТ В РАЗМЕТКЕ СРАЗУ, невыбранная скрыта атрибутом
          `hidden`. Так поисковик видит и текст про съёмку, и текст про
          дизайн: показать ему второй иначе нечем — кликать по развилке он
          не станет. Цена решения названа владелице вслух: скрытый текст
          поиск ценит ниже видимого, и отдельной страницы про дизайн больше
          нет. Она выбрала один адрес, зная это. */}
      <main id="main" className="relative z-10">
        <div hidden={branch !== 'photo'}>
        <Hero />
