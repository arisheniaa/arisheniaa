/**
 * ВЕТКА «ДИЗАЙН» — части одной страницы, а не отдельная страница (Ф72).
 *
 * Была `/design.html` со своей точкой входа. Владелица: «при выборе
 * направления, нужное должно тематически отрисовываться под hero блоком»
 * и «тот же адрес, без перехода». Теперь обе ветки живут на главной, а
 * отсюда экспортируются шапка и содержимое — их вставляет `App.tsx`.
 * Старый адрес перенаправляется на `/#design` (Caddyfile).
 * *
  ...
  return (
    <div className="grid grid-cols-1 items-center gap-x-[var(--gap)] gap-y-[clamp(1.6rem,4vw,2.4rem)] lg:grid-cols-12">
      ...
      <div className={flip ? 'lg:col-span-4 lg:col-start-9 lg:row-start-1' : 'lg:col-span-4 lg:col-start-1'}>
        ...
        {/* НОВАЯ ВКЛАДКА — только чужим сайтам. Своя ветка (`/#main`,
            первая работа) открывается в этой же: уводить человека в новую
            вкладку внутри одного сайта незачем, он и так может вернуться
            назад. Признак — начало адреса, а не отдельное поле в тексте:
            поле пришлось бы держать в согласии с адресом руками. */}
...
/**
 * СОДЕРЖИМОЕ ВЕТКИ — то, что показывается под развилкой при выборе «хочу
 * сайт». Без собственных `<main>` и `<footer>`: на объединённой странице
 * лендмарк один на обе ветки, и заводить второй значило бы сказать
 * скринридеру, что на странице два основных содержания.
 *
 * `id="design"` — адрес начала ветки. Он же метка ветки в адресной строке:
 * `/#design` открывает страницу сразу с дизайном (туда же ведёт
 * перенаправление со старого `/design.html`).
 */
export function DesignSections() {
  return (
    <div id="design" className="scroll-mt-[6rem]">
      <Hero />
      <Works />
      <Cta />
    </div>
