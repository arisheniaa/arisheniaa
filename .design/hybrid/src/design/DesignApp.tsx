 ...
 *
 * Порядок работ — её: мой сайт, затем Алёнин, затем «Элегия». У «Элегии»
 * ссылки нет — сайт не выложен; вместо ссылки метка «скоро откроется».
 *
...
export function DesignApp() {
  return (
    <>
      {/* `veil` — белое дыхание полотна: к середине страницы градиент
          светлеет почти до белого и к низу возвращается в зелёно-жёлтый
          (Ф67, её слова). Формула и обоснование — `.mesh-veil` в
          styles.css; на фотографической ветке пропа нет и слой не
          рендерится. */}
      <Gradient veil />
      {/* Штора развилки: стоит только если сюда пришли с главной через
          «Узнать больше про дизайн», падает сама — см. Gate.tsx. */}
      <Gate page="design" />
      <Page />
      <Grain />
    </>
