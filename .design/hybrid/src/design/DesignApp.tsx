...
function Page() {
  ...
  return (
    <>
      <Header />
      {/* `scroll-mt` — чтобы «Портфолио» из шапки приводило в САМОЕ начало
          страницы, а не к первому её пикселю контента. Без запаса якорь
          отматывал на 73 px (замерено): это высота шапки, и она уезжала за
          верхний край — читатель попадал «в начало», где не видно ни имени,
          ни навигации. Запас больше высоты шапки, поэтому браузер упирается
          в ноль и показывает страницу целиком с её началом. */}
      <main id="main" className="relative z-10 scroll-mt-[6rem]">
        <Hero />
        <Works />
        <Cta />
      </main>
