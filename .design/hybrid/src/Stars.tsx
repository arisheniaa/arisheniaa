...
/* ═══ РОВНОЕ СВЕЧЕНИЕ ТАМ, ГДЕ НЕТ КУРСОРА (Ф55, ИСПРАВЛЕНО Ф56) ══════════
   Владелица, уже после Ф50: «в мобильной версии не горят звёздочки».

   ЗАМЕР СНАЧАЛА, ПРАВКА ПОТОМ. Свечение на телефоне не пропало и даже
   крупнее десктопного: средний радиус ореола 9,9 px против 6,6 на 1280
   (снято чтением `style.filter` у всех групп на обеих ширинах). То есть
   «не горят» — это не про яркость. Это про то, что звёзды на телефоне
   стоят на голом базовом уровне: разгорание привязано к курсору, а курсора
   на тач-вводе нет ни секунды, поэтому `tg` (цель свечения) вечно ноль.
   На ноутбуке же курсор почти всегда где-то на экране, и часть поля всё
   время подсвечена — именно это владелица и читает как «горят».

   ПЕРВАЯ ПОПЫТКА БЫЛА НЕВЕРНОЙ, И ЭТО ЗАПИСАНО, А НЕ ПЕРЕПИСАНО НАЧИСТО.
   Ф55 дала каждой звезде медленный вдох-выдох, рассудив, что глазу не
   хватает движения. Владелица посмотрела и поправила: «ты сделал
   сверкающие мигающие звёздочки, но я хотела чтобы ты сделал их горящими
   как на ноутбуке. они просто загораются сильнее при наведении курсора, но
   сами по себе тоже горящие». То есть на ноутбуке ценно не мерцание, а
   РОВНОЕ горение, поверх которого курсор добавляет вспышку. Пульсация
   снята целиком — вместе с фазой, периодом и синусом.

   СТАЛО: постоянная добавка к свечению там, где курсора не бывает. Ноль на
   десктопе (там добавляет курсор), 0,30 на тач-вводе — это примерно
   середина курсорной вспышки, то самое «подсвечено», что на ноутбуке даёт
   указатель. Звезда получает и больший ореол, и большую плотность:
   у средней звезды (размер ~21 px) радиус ореола становится 16 px против
   прежних 9,7, непрозрачность выше на 0,15.

   ДОБАВКА ЖИВЁТ В ОТРИСОВКЕ, А НЕ В `s.glow`. Это не вкусовое решение:
   `s.glow` участвует в проверке «есть ли что двигать» (`moving` ниже), и
   постоянное ненулевое значение там означало бы, что цикл кадров считает
   поле вечно движущимся и перерисовывает его всегда. Добавка в `paint()`
   оставляет `s.glow` нулём, поэтому фильтр пишется ОДИН раз, а пропуск
   кадров, ради которого Ф45 переписывала полотно, продолжает работать.

   ПРИ ВЫКЛЮЧЕННОМ ДВИЖЕНИИ НИЧЕГО НЕ ТЕРЯЕТСЯ: добавка постоянная, она не
   движение, и применяется в том числе на единственной отрисовке, которой
   ограничивается `prefers-reduced-motion: reduce`. Звёзды там горят так же
   ровно, как и везде. */
const GLOW_STILL = CAN_HOVER ? 0 : 0.3;
  ...
  useEffect(() => {
    ...
    const paint = () => {
      for (let i = 0; i < stars.length; i++) {
        ...
        if (q !== s.glowPainted) {
          s.glowPainted = q;
          /* Постоянная добавка там, где курсора не бывает (`GLOW_STILL`,
             блок вверху файла). Складывается с курсорной вспышкой, а не
             заменяет её: на гибридном планшете с мышью звезда под
             указателем разгорается поверх ровного горения, как и должна. */
          const e = q + GLOW_STILL;
          const base = s.size * GLOW_BASE;
          const blur = base + 22 * e;
          ...
          g.style.opacity = String(Math.min(1, s.alpha + e * 0.5));
        }
      }
    };
