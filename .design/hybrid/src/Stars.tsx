...
/* ═══ СВЕЧЕНИЕ ЗВЁЗД — ОДИН РЕЦЕПТ НА ВСЕ ЭКРАНЫ (Ф50 → Ф55 → Ф57) ════════
   Радиус базового ореола — доля от размера звезды, 0.32, и она ОДНА для
   курсора и для пальца. За тремя правками подряд это число пытались
   развести по типу ввода, и каждый раз становилось хуже. История записана
   целиком, потому что развести его тянет снова при каждой следующей жалобе
   на звёзды.

   Ф50 («звёздочки должны чуть гореть, как на версии на ноутбуке») подняла
   ореол на тач-вводе до 0.46, рассудив так: на ноутбуке курсор половину
   времени кого-то подсвечивает, значит телефону надо добавить постоянно.
   Ф55 («в мобильной версии не горят звёздочки») пошла дальше и дала
   каждой звезде медленное дыхание. Ф56 сняла дыхание по прямой правке
   владелицы («ты сделал сверкающие мигающие звёздочки… я хотела чтобы ты
   сделал их горящими как на ноутбуке») и вместо него добавила плотности —
   лишние тугие слои тени и прибавку к непрозрачности.

   ПОСЛЕ ЭТОГО ВЛАДЕЛИЦА СКАЗАЛА, ЧТО НА САЙТЕ ВСЁ ПО-ПРЕЖНЕМУ. И была
   права: правки шли в неверную сторону, а «доказывались» числом, а не
   кадром. Сравнение наконец сделали по-честному — один и тот же экран
   390 px, те же звёзды, отличается ТОЛЬКО рецепт (`matchMedia` подменён,
   чтобы получить оба рендера рядом). На кадрах видно то, чего не видно в
   числах:

    · рецепт ноутбука — звёзды светлые и полупрозрачные, ореол умеренный;
      они читаются как СВЕТ;
    · рецепт телефона — звёзды темнее и плотнее, бордовые превращаются в
      тяжёлые пятна; они читаются как КЛЯКСЫ.

   Обе прежние догадки были ошибочны, и обе — из-за того, как устроен
   `drop-shadow`. Больший радиус не делает ярче: он размазывает тот же свет
   по большей площади, и каждый пиксель становится ТУСКЛЕЕ — отсюда муть
   у Ф50. Большая непрозрачность не делает светящимся: она делает плотным,
   а свет — это как раз про прозрачность — отсюда тяжесть у Ф56.

   ПОЭТОМУ РАЗВЕДЕНИЯ БОЛЬШЕ НЕТ. Телефон получает буквально тот рецепт,
   который владелица называет правильным, — тот же, что на ноутбуке.
   Курсорная вспышка поверх остаётся там, где есть курсор; на тач-вводе её
   нет, и это не дефект: «сами по себе тоже горящие» — это и есть ровно
   базовый уровень, он же на ноутбуке виден в покое.

   `CAN_HOVER` остался: он всё ещё нужен режиму веера (`Fan.tsx`) и мог бы
   понадобиться здесь снова, но НА СВЕЧЕНИЕ БОЛЬШЕ НЕ ВЛИЯЕТ. Самопроверка
   сверяет строку фильтра при подменённом `matchMedia` в обе стороны — если
   рецепты когда-нибудь снова разъедутся, она это покажет. */
const CAN_HOVER =
    ...
    /* Ф50: «звёздочки должны чуть гореть, как на версии на ноутбуке».
       ...
       `drop-shadow` на месте. Разница в другом, и она двойная:
        ...
        2. НА ТАЧЕ НЕТ КУРСОРА, А ЗНАЧИТ НЕТ И ВСПЫШЕК. На ноутбуке звезда
           ...
           существует, звёзды вечно на базовом уровне.

           ВЫВОД ИЗ ЭТОГО ПУНКТА ОКАЗАЛСЯ НЕВЕРНЫМ, см. блок «СВЕЧЕНИЕ
           ЗВЁЗД — ОДИН РЕЦЕПТ» ниже: поднимать базовый уровень на тач-вводе
           не следовало, от этого звёзды стали мутнее, а не ярче. Сам пункт
           оставлен как есть — он верно описывает, ПОЧЕМУ телефон выглядит
           иначе; неверным был предложенный ответ, а не наблюдение. Число
           кадров (вдвое, а не втрое) из пункта 1 в силе. */
