...
type Star = {
  ...
  glowPainted: number;
  /** сдвиг фазы дыхания, 0..2π — чтобы поле не мигало разом (см. `BREATH_*`) */
  phase: number;
  /** длительность одного вдоха-выдоха, с; своя у каждой звезды */
  period: number;
};
...
/* ═══ ДЫХАНИЕ СВЕЧЕНИЯ ТАМ, ГДЕ НЕТ КУРСОРА (Ф55) ═════════════════════════
   ...
   «не горят» — это не про яркость. Это про то, что свечение МЁРТВОЕ:
   на ноутбуке звезда под курсором разгорается, подрастает и гаснет, поле
   всё время дышит, и глаз читает именно эту жизнь как «горят». На тач-вводе
   курсора нет ни секунды — `tg` (цель свечения) вечно ноль, каждая звезда
   намертво стоит на базовом уровне. Ф50 подняла базу и этим сделала поле
   ярче, но неподвижным оно и осталось: подняли громкость там, где не хватало
   не громкости, а движения.

   ПОЭТОМУ ЗДЕСЬ НЕ ЯРКОСТЬ, А ФАЗА. Каждая звезда получает собственный
   медленный вдох-выдох: цель свечения ходит между 0,06 и 0,34 — то есть
   заметно ниже полной курсорной вспышки (1,0), это тлеющий огонёк, а не
   мигалка. Фаза и период у каждой свои, иначе поле моргало бы разом, как
   гирлянда.

   ЧИСЛА. Период 4,5–7,5 с — вдох медленнее, чем взгляд задерживается на
   одной точке, поэтому движение читается боковым зрением и не перетягивает
   внимание с текста. Верх 0,34 выбран не на глаз: при нём радиус ореола у
   средней звезды (размер ~21 px) ходит от 10,5 до 18 px, то есть примерно
   тот же размах, что даёт курсор на ноутбуке между «рядом» и «под самой
   звездой», — телефон получает ту же амплитуду жизни, а не свою.

   ТОЛЬКО ТАМ, ГДЕ КУРСОРА НЕ БЫВАЕТ (`!CAN_HOVER`). На десктопе дыхание
   спорило бы с курсорной вспышкой: две причины на одно свойство, и звезда
   под указателем разгоралась бы не всегда одинаково. Десктоп не тронут ни
   одним числом.

   ПРИ ВЫКЛЮЧЕННОМ ДВИЖЕНИИ ДЫХАНИЯ НЕТ, и для этого не понадобилось ни
   одной строки: при `prefers-reduced-motion: reduce` цикл кадров вообще не
   запускается (см. ранний выход ниже), звёзды остаются на базовом уровне —
   то есть ровно там, где они и были до этой правки. */
const BREATH_LOW = 0.06;
const BREATH_HIGH = 0.34;
  ...
  const stars = useMemo<Star[]>(() => {
    ...
    return cells.slice(0, n).map(([c, r]) => {
      ...
      return {
        ...
        glow: 0,
        glowPainted: -1,
        // из того же детерминированного `rand()`, что и всё остальное:
        // россыпь обязана быть одинаковой при каждой загрузке
    ...
    const loop = (t: number, dt: number) => {
      ...
      for (let i = 0; i < stars.length; i++) {
        ...
        if (hasPointer) {
          ...
          /* Тлеющий огонёк вместо мёртвого уровня — см. блок «ДЫХАНИЕ
             СВЕЧЕНИЯ» вверху файла. Ветка `else` не случайна: если на
             гибридном устройстве (планшет с мышью) курсор всё-таки появился,
             ведёт он, а не дыхание — две причины на одно свойство не
             складываются. */
          tg = BREATH_LOW + (BREATH_HIGH - BREATH_LOW) * (0.5 + 0.5 * Math.sin((t / s.period) * TAU + s.phase));
        }
