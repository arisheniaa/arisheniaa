...
/* Само число 0.32 пережило все перечисленные правки и живёт дальше — но уже
   не как радиус размытия, а как вынос ореола-фигуры, см. `GLOW_R` ниже.
   Отдельной константы больше нет: у неё остался ровно один потребитель. */

/* ═══ ОРЕОЛ — ФИГУРА, А НЕ ФИЛЬТР (Ф58) ══════════════════════════════════
   Владелица прислала кадр с iPhone: сами звёзды на месте и правильного
   цвета, а ореола вокруг них НЕТ ВОВСЕ. Не слабее и не размытее — его нет.
   Это объясняет, почему ни одна правка Ф50/Ф55/Ф56/Ф57 на её телефоне
   ничего не меняла: все они крутили параметры эффекта, который на iOS не
   применяется.

   ПОЧЕМУ. Свечение было CSS-фильтром `drop-shadow` на SVG-группе. У WebKit
   давняя особенность: такой фильтр на элементе ВНУТРИ SVG обрезается по
   границам самой фигуры, и весь вынос ореола срезается. Десктопная сборка
   WebKit, на которой я проверял, этого не делает — потому дефект и не
   ловился ничем, кроме живого телефона.

   ЧТО ДОКАЗЫВАЕТ КАДР: путь звезды рисуется и берёт цвет из переменной
   (`fill="var(--star-…)"`) — значит и SVG, и переменные на iOS работают.
   Не работает ровно фильтр. Поэтому ореол переведён на тот же механизм,
   который на её телефоне заведомо рисует: обычная фигура с заливкой.

   КРУГ С РАДИАЛЬНЫМ ГРАДИЕНТОМ вместо размытия. Радиус 82 в местной
   системе координат: путь звезды занимает 0..100, то есть её собственный
   радиус около 50, а прежний ореол выносился на 0,32 размера, то есть на 32
   единицы за край — сумма и даёт 82. Плотность 0.85 подобрана так, чтобы
   на кремовом полотне ореол читался, но не спорил с текстом.

   Курсорная вспышка выражена теми же двумя свойствами, что и раньше, но
   через фигуру: радиус растёт на 90 %, плотность — на 110 %. У C под
   курсором звезда разгоралась примерно вдвое, и это то же самое, просто
   размером фигуры, а не радиусом размытия.
...
export function Stars({
  ...
  /* Префикс для идентификаторов градиентов. `useId()` даёт значение,
     одинаковое на сервере и в браузере, — важно, потому что несовпадение
     идентификатора между разметкой и гидратацией оставило бы ссылки
     `url(#…)` висеть в пустоту. Двоеточия из значения убраны: в CSS-селекторе
     и в `url(#…)` они требуют экранирования. */
  ...
  useEffect(() => {
    ...
    const glows = groups.map((g) => g.querySelector<SVGCircleElement>('[data-glow]'));
    ...
    const paint = () => {
      for (let i = 0; i < stars.length; i++) {
        ...
        /* Свечение. Квант 0.08: ниже него ореол переписывался бы каждый кадр
           без видимого изменения. */
        const q = Math.round(s.glow / 0.08) * 0.08;
        if (q !== s.glowPainted) {
          s.glowPainted = q;
          /* Ореол — отдельный круг под звездой (`data-glow`), а не фильтр:
             курсорная вспышка меняет его плотность и радиус. Числа —
             продолжение прежней логики: у C под курсором звезда разгоралась
             примерно вдвое и раздувала ореол, здесь то же самое, только
             выражено размером фигуры, а не размытием. */
          const halo = glows[i];
          if (halo) {
            halo.setAttribute('opacity', (GLOW_OPACITY * (1 + q * 1.1)).toFixed(3));
            halo.setAttribute('r', (GLOW_R * (1 + q * 0.9)).toFixed(1));
          }
  ...
  return (
    ...
    >
      <svg ref={svg} width="100%" height="100%" focusable="false">
        {/* Один градиент на тон, а не на звезду: тонов три, звёзд бывает
            под сотню. Идентификаторы с префиксом от `useId()` — на странице
            шесть россыпей, и без префикса их определения перебили бы друг
            друга общим именем.

            Цвет стопа задан ровно так же, как заливка самой звезды
            (`var(--star-…)` в атрибуте) — тем механизмом, который на iPhone
            владелицы доказанно работает: на её кадре пути окрашены верно. */}
        <defs>
          {ALL_TONES.map((t) => (
        ...
        {stars.map((s, i) => (
          ...
          <g key={i} data-star={i} style={{ opacity: s.alpha }}>
            {/* Ореол ПЕРЕД путём — значит рисуется под ним. Центр 50/50: путь
                звезды живёт в сетке 0..100, и круг обязан стоять в той же
                системе координат, иначе свечение съедет при повороте. */}
            <circle
              data-glow
              cx="50"
              cy="50"
              r={GLOW_R}
              fill={`url(#${uid}-glow-${s.tone})`}
