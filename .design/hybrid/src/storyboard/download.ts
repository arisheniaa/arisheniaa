...
export async function renderStoryboardCanvas(photos: StoryboardPhoto[]): Promise<HTMLCanvasElement> {
  ...
  /* Ф41 меняла заголовок на экране («вместо фраз … напиши просто одну
     "Наша возможная раскадровка"»). Про скачиваемый файл речи не было, но
     это ТА ЖЕ фраза и тот же документ: человек уносит PDF в переписку, и
     разойдись он с экраном — вышло бы, что раскадровка называется по-разному
     до и после нажатия «Скачать». Решение билдера, откат — одна строка. */
  ctx.fillText('Наша возможная раскадровка', PAD, PAD + 38);
